...
impl App {
    ...
    pub async fn get_contents(&mut self, uid: &str) -> Result<Vec<Message>> {
        ...
        for m in msg {
            ...
            if !existed {
                self.db
                    .add_contents(uid, &m.link, &m.link, &m.description, false)
                    .await
                    .unwrap();
                ret.push(m);
            }
    ...
    pub async fn publish(&mut self, user_name: &str, message: &str) -> Result<bool> {
        let secret_key = self
            .db
            .find_user_private_key(user_name)
            .await
            .unwrap()
            .unwrap();
        let avatar = self.db.quert_avatar(user_name).await.unwrap().unwrap();
        let key = self.convert_key(&secret_key)?;
        let note_publish = NotePublisher::new(&key, CONF_PATH).await?;
        note_publish.connect().await;
        let _ = note_publish.set_metadata(user_name, &avatar);
